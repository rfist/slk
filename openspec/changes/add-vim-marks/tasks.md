...
- [x] 1.1 Add a `Location` struct (team ID, channel ID, message TS,
...
- [x] 1.2 Add a helper that builds a `Location` from the app's current
...
- [x] 2.3 Leave `completePendingLinkNav` and `openThreadForPermalink`
      working exactly as they do now, including the `FetchAround`
      fallback and the stale-navigation drop.
- [x] 2.4 Run `go test ./... -race`. The existing permalink navigation
...
- [x] 3.1 Change `navStack.entries` in `internal/ui/navhistory.go` from
      `[]string` to a slice of `Location`. Keep `navStackMax = 50`, the
      forward-path truncation, and the cursor semantics unchanged.
- [x] 3.2 Change `navHistoryStore.Push` to take a `Location`. Push
...
- [x] 3.3 Add `navHistoryStore.UpdateCurrent(teamID string, loc
...
- [x] 3.4 In `internal/ui/reducer_channels.go`, capture the departing
      ...
      skip a channel.
- [x] 3.5 Acceptance check for 3.1-3.4: **every existing navhistory
      ...
      tests are stale.
- [x] 3.6 Change `navHistoryStore.Walk` to return a `Location`. Keep
      the stale-entry skip-and-drop behaviour for entries whose
      *channel* does not resolve via `ChannelLookupFunc`.
- [x] 3.7 Change `a.navigateBack` / `a.navigateForward`
      ...
      not grow the stack.
- [x] 3.8 In `walkNavCmd` (`internal/ui/app.go`), call `UpdateCurrent`
...
- [x] 3.9 Tests for 3.8: walk back to a channel, move to a different
      ...
      first walk.
- [x] 3.10 A walked location whose message cannot be found must open the
...
- [x] 3.11 Tests -- these are the ones that matter most in this change:
...
- [x] 4.2 Call it from the thread-opening path so a location naming a
      reply lands on that reply rather than the top of the thread.
      `openThreadForPermalink` (`internal/ui/reducer_links.go:117`) is
      the entry point.
- [x] 4.3 Tests: selecting an existing reply returns true and moves the
...
- [x] 5.1 Add a `marks` table to the schema string in
...
- [x] 5.2 Add accessors: upsert a mark, list marks for a workspace,
...
- [x] 5.3 Add the in-memory session tier: a per-workspace map for
      lowercase marks.
- [x] 5.4 Add the merged accessor the UI reads through, so no caller
      ...
      is on.
- [x] 5.5 Tests: uppercase survives a simulated restart, lowercase does
...
- [x] 6.1 Add a pending-key flag for `m` on `App`, following
...
- [x] 7.1 Add the pending-key flag for `'` in the same shape as group
      6, plus backtick as an alias.
- [x] 7.2 On a letter, look up the mark and hand its `Location` to the
      ...
      were.
- [x] 7.3 An unset letter changes nothing and toasts.
- [x] 7.4 A mark whose team ID is not the active workspace does not
...
- [x] 7.5 A mark whose message cannot be found opens the channel,
...
- [x] 7.6 Tests: jump into another channel selects the message; jump
...
- [x] 7.7 Add a back-jump slot to `App`: one `Location` plus a bool (or
...
- [x] 7.8 Write the slot with the current location immediately before a
      ...
      nothing.
- [x] 7.9 Bind the back-jump: `''` and backtick-backtick, i.e. the mark
...
- [x] 7.10 Tests, one per scenario in the spec's "A mark jump is
...
- [x] 8.1 Add `internal/ui/marks/` as a self-contained widget package
...
- [x] 8.2 Add `ModeMarks` to `internal/ui/mode.go`, include it in
      `IsModalOverlay`, and give it a `String()` label.
- [x] 8.3 Render one row per mark: letter, channel, message preview,
      all from the stored snapshot. No fetching -- the overlay must
      draw correctly offline and immediately after a restart.
- [x] 8.4 Selecting a row closes the overlay and jumps to that mark via
      the group-7 path.
- [x] 8.5 Add the delete action on the selected row, calling the same
      removal the `:delmarks` command uses.
- [x] 8.6 Empty state: report that no marks are set.
- [x] 8.7 Open the overlay when `'` is armed, unless the config option
...
- [x] 8.8 Tests: rows render from the snapshot with no service calls;
...
- [x] 9.1 Register `marks` in the `commands` map in
      `internal/ui/command.go`. It always opens the overlay,
      regardless of the jump-overlay config option.
- [x] 9.2 Register `delmarks`, taking one or more letters. Deleting an
      unset letter is harmless.
- [x] 9.3 Tests: `:marks` opens the overlay with the option off;
...
- [x] 10.1 Add a `Marks` struct to `internal/config/config.go` as a
      `[marks]` block beside `General` / `Appearance` / `Sidebar`,
      with the persist-all option (default off) and the jump-overlay
      option (default on).
- [x] 10.2 Wire both options through to the storage tier and the `'`
      handler.
- [x] 10.3 Tests: unset config yields lowercase-temporary and
      overlay-on; each option honoured when set.

## 11. Finish

- [x] 11.1 Run `go build ./...`, `go test ./... -race`, `go vet ./...`,
      and `gofmt -l` on changed files only.
- [x] 11.2 Add the new keys to the help overlay so `?` lists `m`, `'`,
      `:marks`, and `:delmarks`.
- [x] 11.3 Document the marks keys, the two commands, and the `[marks]`
      config block in the wiki, matching where the window commands and
      presence are documented.
- [x] 11.4 Log the decisions and anything surprising in
      `devdocs/fyi.md`.
