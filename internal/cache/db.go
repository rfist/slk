...
func (db *DB) migrate() error {
	...
	// full.
	hadStatus, err := db.hasColumn("users", "status_emoji")
	if err != nil {
		return err
	}
	...
	// The same one-time version reset, keyed on its own column, so a
	// database that already gained the status columns refetches too.
	hadHuddle, err := db.hasColumn("users", "huddle_state")
	if err != nil {
		return err
	}
